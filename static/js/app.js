...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    let filteredSamples = data.samples.find(obj => obj.id == sample);
    ...
    let bubbleLayout = {
      title: 'Bacteria Cultures Per Sample',  // Add bubble chart title 
      xaxis: { title: 'Sample Values' },  // add x-axis title
      yaxis: { title: 'OTU ID' },
      showlegend: false
    };
    ...
    let barLayout = {
      title: 'Top 10 Bacteria Cultures Found',  // bar chart title
      xaxis: { title: 'Number of Bacteria' },  //  x-axis title
      yaxis: { title: 'OTU IDs' }
    };
